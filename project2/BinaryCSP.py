...
def consistent(assignment, csp, var, value):
    """Question 1"""
    """YOUR CODE HERE"""
    if not assignment.assignedValues.get(var)  == None:
        return False
    else:
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
    ...
    """YOUR CODE HERE"""
    initialDomains = {}
    for node in assignment.varDomains.keys():
        initialDomains[node] = assignment.varDomains[node]
    return recursiveBacktracking2(initialDomains, assignment, csp, orderValuesMethod, selectVariableMethod)

def recursiveBacktracking2(initialDomains, assignment, csp, orderValuesMethod, selectVariableMethod):
    ...
    for value in orderValuesMethod(assignment, csp, currentVar):
        if consistent(assignment, csp, currentVar, value):
            assignment.varDomains[currentVar] = set([value])
            assignment.assignedValues[currentVar] = value
            returnAssignment = recursiveBacktracking2(initialDomains, assignment, csp, orderValuesMethod, selectVariableMethod)
            if not returnAssignment == None:
                return returnAssignment
        assignment.varDomains[currentVar] = initialDomains[currentVar]
...
def minimumRemainingValuesHeuristic(assignment, csp):
    """Question 2"""
    """YOUR CODE HERE"""
    domains = assignment.varDomains.items()
    domains2 = []
    for pair in domains:
        if not assignment.isAssigned(pair[0]):
            domains2.append(pair)
    minVal = domains2[0]
    for pair in domains2:
        if len(pair[1]) < len(minVal[1]):
            minVal = pair
    minList = []
    minList.append(minVal)
    for pair in domains2:
        if len(pair[1]) == len(minVal[1]) and not pair == minVal:
            minList.append(pair)
    if len(minList) == 1:
        nextVar = minList[0][0]
    else:
        ...
        for pair in minList:
            for constraint in csp.binaryConstraints:
                if constraint.affects(pair[0]) and not assignment.isAssigned(constraint.otherVariable(pair[0])):
                    if constraintDict.has_key(pair[0]):
                        constraintDict[pair[0]] = constraintDict[pair[0]] + 1
                    else:
                        constraintDict[pair[0]] = 1
        if len(constraintDict) == 0:
            return minList[0][0]
        else:
            constraintList = constraintDict.items()
            maxVal = constraintList[0]
            for pair in constraintList:
                if pair[1] > maxVal[1]:
                    maxVal = pair
            nextVar = maxVal[0]
    return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    constraintList = []
    for value in values:
        constraintList.append((value, constrainedValueCount(assignment, csp, var, value)))
    constraintList = sorted(constraintList, key=lambda i:i[1])
    constraintList.reverse()
    returnList = []
    for constraint in constraintList:
        returnList.append(constraint[0])
    return returnList

def constrainedValueCount(assignment, csp, var, value):
    count = 0
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            for val in assignment.varDomains[constraint.otherVariable(var)]:
                if not val == value:
                    count = count + 1
    return count
"""
...
def forwardChecking(assignment, csp, var, value):
    ...
    inferences = set([])
    for constraint in csp.binaryConstraints:
        if constraint.affects(var) and not inferences == None:
            newSet = set([])
            for val in assignment.varDomains[constraint.otherVariable(var)]:
                if not val == value:
                    newSet.add(val)
                else:
                    inferences.add((constraint.otherVariable(var), value))
            if len(newSet) == 0:
                inferences = None
            else: assignment.varDomains[constraint.otherVariable(var)] = newSet
    if inferences == None:
        for node in assignment.varDomains.keys():
            assignment.varDomains[node] = initialDomains[node]
    return inferences
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    for node in assignment.varDomains.keys():
        initialDomains[node] = assignment.varDomains[node]
    return recursiveBacktracking3(initialDomains, assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)

def recursiveBacktracking3(initialDomains, assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    if assignment.isComplete():
        return assignment
    currentVar = selectVariableMethod(assignment, csp)
    for value in orderValuesMethod(assignment, csp, currentVar):
        giveBack = set([])
        if consistent(assignment, csp, currentVar, value):
            giveBack = inferenceMethod(assignment, csp, currentVar, value)
            if not inferenceMethod(assignment, csp, currentVar, value) == None:
                assignment.assignedValues[currentVar] = value
                returnAssignment = recursiveBacktracking3(initialDomains, assignment, csp, orderValuesMethod,
                                                      selectVariableMethod, inferenceMethod)
                if not returnAssignment == None:
                    return returnAssignment
                else:
                    for unit in giveBack:
                        assignment.varDomains[unit[0]].add(unit[1])
                assignment.assignedValues[currentVar] = None
    return None
    ...
    if constraint.affects(var1) and constraint.affects(var2):
        for value2 in assignment.varDomains[var2]:
            ...
            for value1 in assignment.varDomains[var1]:
                if not value1 == value2 and len(assignment.varDomains[var2]) > 0:
                    consistency = True
            if consistency == False:
                inferences.add((var2, value2))
                newSet.remove(value2)
        assignment.varDomains[var2] = newSet
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var) and not assignment.isAssigned(constraint.otherVariable(var)):
            arcQueue.put((var, constraint.otherVariable(var), constraint))
    while not arcQueue.empty() and not inferences == None :
        currentArc = arcQueue.get()
        giveBack = revise(assignment, csp, currentArc[0], currentArc[1], currentArc[2])
        if giveBack == None:
            for unit in inferences:
                assignment.varDomains[unit[0]].add(unit[1])
            inferences = None
        else:
            if len(giveBack) > 0:
                for constraint in csp.binaryConstraints:
                    if constraint.affects(currentArc[1]) and not assignment.isAssigned(constraint.otherVariable(currentArc[1])):
                        arcQueue.put((currentArc[1], constraint.otherVariable(currentArc[1]), constraint))
                for unit in giveBack:
                    inferences.add(unit)
    return inferences
...
def AC3(assignment, csp):
    ...
    for constraint in csp.binaryConstraints:
        for var in csp.varDomains.keys():
            if constraint.affects(var):
                arcQueue.put((var, constraint.otherVariable(var), constraint))
    while not arcQueue.empty() and not assignment == None :
        currentArc = arcQueue.get()
        giveBack = revise(assignment, csp, currentArc[0], currentArc[1], currentArc[2])
        if giveBack == None:
            for unit in inferences:
