...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        successorDict = {}
        for action in legal:
            successorDict[action] = Actions.getSuccessor(pacmanPosition,action)
        mostLikelyPositions = []
        for ghost in livingGhostPositionDistributions:
            maxProb = 0.0
            for key,value in ghost.iteritems():
                if value > maxProb:
                    maxProb = value
                    maxPosition = key
            mostLikelyPositions.append((maxPosition, maxProb))
        minDistanceFromPacman = 0
        firstIteration = True
        for mostLikelyPosition in mostLikelyPositions:
            if firstIteration:
                minPositionFromPacman = mostLikelyPosition
                minDistanceFromPacman = self.distancer.getDistance(pacmanPosition, mostLikelyPosition[0])
                firstIteration = False
            else:
                distance = self.distancer.getDistance(pacmanPosition, mostLikelyPosition[0])
                if distance < minDistanceFromPacman:
                    minDistanceFromPacman = distance
                    minPositionFromPacman = mostLikelyPosition
        returnAction = None
        firstIteration = True
        minDistanceFromPacman2 = 0
        for key,value in successorDict.iteritems():
            if firstIteration:
                returnAction = key
                minDistanceFromPacman2 = self.distancer.getDistance(minPositionFromPacman[0], value)
                firstIteration = False
            else:
                distance = self.distancer.getDistance(minPositionFromPacman[0], value)
                if distance < minDistanceFromPacman2:
                    returnAction = key
                    minDistanceFromPacman2 = distance
        return returnAction
