        ...
        if not noisyDistance == None:
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    noisyGivenTrue = emissionModel[trueDistance]
                    allPossible[p] = noisyGivenTrue * self.beliefs[p]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, newGivenOld in newPosDist.items():
                oldAndNew = oldProb * newGivenOld
                allPossible[newPos] = allPossible[newPos] + oldAndNew
        allPossible.normalize()
        self.beliefs = allPossible
    def getBeliefDistribution(self):
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            count = 0
            while count < len(self.particleList):
                self.particleList[count] = self.getJailPosition()
                count+=1
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particleList:
            if beliefDistribution[particle] == None:
                beliefDistribution[particle] = 0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while counter < self.numParticles:
            ...
            while ghostCount <= self.numGhosts:
                ghostList.append(self.legalPositions[i])
                ghostCount+=1
            self.particleList.append(tuple(ghostList))
            i+=1
            counter+=1
            if i == numLegalPositions:
                i = 0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        allPossible = []
        while ghostCount < self.numGhosts:
            allPossible.append(util.Counter())
            ghostCount+=1
        ghostCount = 0
        while ghostCount < self.numGhosts:
            sum = 0
            for key,value in allPossible[ghostCount].iteritems():
                sum += value
            while sum == 0 and not noisyDistances[ghostCount] == None:
                for particle in self.particleList:
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[ghostCount])
                    allPossible[ghostCount][particle[ghostCount]] = allPossible[ghostCount][particle[ghostCount]] + emissionModels[ghostCount][trueDistance]
                    sum = 0
                    for key, value in allPossible[ghostCount].iteritems():
                        sum += value
                    if sum == 0 and not noisyDistances[ghostCount] == None:
                        self.initializeParticles()
            ghostCount+=1
        ...
        while particleCount < self.numParticles:
            ...
            ghostCount = 0
            while ghostCount < self.numGhosts:
                if not noisyDistances[ghostCount] == None:
                    updatedParticle.append(util.sample(allPossible[ghostCount]))
                else:
                    updatedParticle.append(self.getJailPosition(ghostCount))
                ghostCount+=1
            tupledParticle = tuple(updatedParticle)
            updatedArray.append(tupledParticle)
            particleCount+=1
        self.particleList = updatedArray
