...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        counter = 0
        differenceList = []
        while counter < len(newInActs):
            currentWeight = self.weights[counter]
            difference = alpha * delta * newInActs[counter]
            differenceList.append(difference)
            newWeight = currentWeight + difference
            self.weights[counter] = newWeight
            counter +=1
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        layerCount = 0
        while layerCount < self.numLayers:
            new = []
            for perceptron in self.layers[layerCount]:
                sigmoidOutput = perceptron.sigmoidActivation(current)
                new.append(sigmoidOutput)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) ##filled in sigmoidActivationDeriv
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) ##filled in sigmoidActivationDeriv
                    deltaSum = 0.0
                    index = 0
                    for perceptron in nextLayer:
                        deltaSum += perceptron.weights[neuronNum+1] * deltas[0][index]
                        index +=1
                    delta = deltaSum * gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while (weightMod > weightChangeThreshold) and (iteration < maxItr):
        outputTuple = nnet.backPropLearning(examplesTrain, alpha)
        avgError = outputTuple[0]
        avgWeightChange = outputTuple[1]
        weightMod = avgWeightChange
    ...
    testError = 0
    testCorrect = 0
    numTests = len(examplesTest)

    testAccuracy = 0  # num correct/num total

    for example in examplesTest:
        input = example[0]
        expectedOut = example[1]
        output = nnet.feedForward(input)
        lastOut = output[-1]
        roundedOut = []
        for value in lastOut:
            roundedOut.append(round(value))
        if roundedOut == expectedOut:
            testCorrect += 1
        else:
            testError += 1
    testAccuracy = testCorrect/float(numTests)
